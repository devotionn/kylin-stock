...
名称相同、规格不同的物资不允许被 OCR 自动猜成同一个物资。
...
export KYLIN_STOCK_OCR_PYTHON="$HOME/.local/share/kylin-stock/ocr-venv/bin/python"
```
...
- 目标 ARM64 包仍能通过现有 native package smoke build。
