...
const DocumentImportView = () => import('../views/DocumentImportView.vue')
...
const routes = [
  ...
  { path: '/materials', component: MaterialsView, meta: { title: '物资管理', subtitle: '维护物资名称、规格型号、单位和存放位置' } },
  { path: '/stock-in', component: StockOperationView, meta: { title: '入库登记', subtitle: '登记物资入库并自动增加库存' } },
  { path: '/document-import', component: DocumentImportView, meta: { title: '扫描单据导入', subtitle: '本地识别调拨接收单，人工确认后整单入库' } },
  ...
  { path: '/ledger', component: LedgerView, meta: { title: '出入库明细', subtitle: '按名称、规格、时间、单位和去向查询流水' } },
